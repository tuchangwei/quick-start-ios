...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // Register to hanlde Layer Notifications
...
- (void)setupNotificationObservers
{
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleAuthenticationChallenge:)
                                                 name:LayerDidReceiveAuthenticationChallenge
                                               object:nil];
}

- (void)handleAuthenticationChallenge:(NSNotification *)notification
{
    NSString *nonce = [notification.userInfo valueForKey:LayerAuthenticationChallengeNonce];
    [self requestIdentityTokenForUserID:kUserID appID:[self.layerClient.appID UUIDString] nonce:nonce completion:^(NSString *identityToken, NSError *error) {
...
- (void)showFirstTimeMessage;
{
    if (![[NSUserDefaults standardUserDefaults] boolForKey:ApplicationHasLaunchedOnce])
    {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Hello!"
                                                        message:@"This app is a very simple chat app using Layer.  Launch this app on a Simulator and a Device to start a 1:1 conversation."
                                                       delegate:self
